...
class NavigatorUtils {
  ...
  static push(BuildContext context, String path,
      {bool replace = false, bool clearStack = false, Map<String, String> params}) {
    FocusScope.of(context).requestFocus(new FocusNode());
    String paramsStr = _parseParams(params);
    if(isNotEmpty(paramsStr)) {//有参数传递
      path += paramsStr;
    }
    GlobalRouter.router.navigateTo(context, path, replace: replace, clearStack: clearStack, transition: TransitionType.inFromRight);
  }

  static pushResult(BuildContext context, String path, Function(Object) function,
      {bool replace = false, bool clearStack = false}) {
    FocusScope.of(context).requestFocus(new FocusNode());
    GlobalRouter.router.navigateTo(context, path, replace: replace, clearStack: clearStack, transition: TransitionType.inFromRight).then((result){
      // 页面返回result为null
      if (result == null){
        return;
      }
  ...
  /// 跳到带返回按钮的WebView页
  static goWebViewPageCanGoBack(BuildContext context, String title, String url){
